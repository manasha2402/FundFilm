  ...
  async (req, res) => {
    try {
      ...
      if (req.files?.image?.[0] && existingUpdate.image) {
        const oldImagePath = path.join("uploads", path.basename(existingUpdate.image));
      ...
      if (req.files?.modal_image?.[0] && existingUpdate.modal_image) {
        const oldModalImagePath = path.join("uploads", path.basename(existingUpdate.modal_image));
...
router.delete("/updates/:id", async (req, res) => {
  try {
    ...
    if (update.modal_image) {
      fs.unlink(path.join("uploads", path.basename(update.modal_image)), (err) => {
